    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        whatsappButtons.forEach(btn => {
            btn.addEventListener('click', function() {
                const telefono = this.dataset.telefono.replace(/\D/g, ''); // Limpiar el número (solo dígitos)
                const nombre = this.dataset.nombre.split(' ')[0]; // Usar solo el primer nombre para ser más cercano
                ...
                // LÓGICA DEL "CHATBOT": Construir mensaje según contexto
                if (proximaCita && proximaCita.length > 5) {
                    // CASO 1: Tiene cita próxima -> Mensaje de Recordatorio
                    mensaje = `${saludo} ${nombre}, le escribimos del Consultorio Dental RC para recordarle su cita programada para el día *${proximaCita}*. Favor de confirmar su asistencia. 🦷✨`;
                } else {
                    // CASO 2: No tiene cita -> Mensaje de Seguimiento / Promoción
                    mensaje = `${saludo} ${nombre}, esperamos que se encuentre bien. Le escribimos del Consultorio Dental RC para agendar su revisión periódica y mantener su sonrisa saludable. ¿Le gustaría ver horarios disponibles? 📅`;
                }
