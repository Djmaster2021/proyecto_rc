...
from domain.models import Servicio, Disponibilidad
...
def api_slots_disponibles(request):
    ...
    # ---------------- Resolver dentista ----------------
    if dentista_id:
        # Usamos Disponibilidad para encontrar un dentista con ese ID
        disp = Disponibilidad.objects.filter(dentista_id=dentista_id).first()
        if not disp:
            return JsonResponse(
                {"detail": "Dentista no encontrado."},
                status=404,
            )
        dentista = disp.dentista
    else:
        dentista = getattr(request.user, "perfil_dentista", None)
        if dentista is None:
            return JsonResponse(
